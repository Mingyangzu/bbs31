var IMGFOLDERPATH = 'source/plugin/it618_storemapad/win/images/';//图片路径配置
...
var $it618E = {};
$it618E.$it618A = function(attr,ele) {
	ele = ele || this;
	ele = $it618(ele);
	return ele.getAttribute?ele.getAttribute(attr):null;
}
$it618E.getTopLevelWindow = function(){
	var pw = window;
	while(pw!=pw.parent){
		pw = pw.parent;
	}
...
$it618E.hide = function(ele) {
	ele = ele || this;
	ele = $it618(ele);
  ele.style.display = 'none';
}
$it618E.show = function(ele) {
	ele = ele || this;
	ele = $it618(ele);
  ele.style.display = '';
}
$it618E.visible = function(ele) {
	ele = ele || this;
	ele = $it618(ele);
...
Core.attachMethod = function(ele){
	...
	try{
		...
		for(var prop in $it618E){
			ele[prop] = win.$it618E[prop];
		}
	}catch(ex){
...
Dialog.prototype.it618_showWindow = function(){
	if(isIE){
...
Dialog.prototype.show = function(){
	var pw = $it618E.getTopLevelWindow();
	var doc = pw.document;
...
if ( !this.ParentWindow ){
	...
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='$it618(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var bgdiv = pw.$it618("_DialogBGDiv");
	if(!bgdiv){
		bgdiv = pw.document.createElement("div");
		bgdiv.id = "_DialogBGDiv";
		$it618E.hide(bgdiv);
	 	pw.$it618T("body")[0].appendChild(bgdiv);
		if(isIE6){
	...
	var div = pw.$it618("_DialogDiv_"+this.ID);
	if(!div){
		...
		//div.setAttribute("dragStart","Dialog.dragStart");
	 	pw.$it618T("body")[0].appendChild(div);
	}
	/*div.onmousedown = function(evt){
		var w = $it618E.getTopLevelWindow();
	...
	if(this.innerElementId){
		var innerElement=$it618(this.innerElementId);
		...
		if(isIE){
			...
			pw.$it618("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			pw.$it618("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	pw.$it618("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		pw.$it618("_DialogFrame_"+this.ID).DialogInstance = this;
	pw.Drag.init(pw.$it618("_draghandle_"+this.ID),pw.$it618("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		pw.$it618("_DialogDiv_"+this.ID).dialogId=this.ID;
		pw.$it618("_DialogDiv_"+this.ID).onDragStart = function(){$it618("_Covering_"+this.dialogId).style.display=""}
		pw.$it618("_DialogDiv_"+this.ID).onDragEnd = function(){$it618("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = pw.$it618("_ButtonOK_"+this.ID);
	this.CancelButton = pw.$it618("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		$it618E.show(pw.$it618("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			pw.$it618("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			pw.$it618("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		pw.$it618("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=$it618E.getTopLevelWindow().$it618(this.innerElementId);
		...
		if(isIE){
			...
			fragment.innerHTML=innerElement.outerHTML;
			innerElement.outerHTML="";
			$it618T("body")[0].appendChild(fragment)
		}else{
			$it618T("body")[0].appendChild(innerElement)
		}

	}
	if(this.WindowFlag){
		this.ParentWindow.$it618D = null;
		this.ParentWindow.$it618DW = null;
		...
		if(this.AlertFlag){
			$it618E.hide(pw.$it618("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = $it618E.getTopLevelWindow();$it618E.hide(w.$it618("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = $it618E.getTopLevelWindow();
	pw.$it618("_DialogButtons_"+this.ID).$it618T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	pw.$it618("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	pw.$it618("_AlertBGDiv").style.display="";
	$it618E.hide(pw.$it618("_ButtonOK_"+diag.ID));
	var win = pw.$it618("_DialogFrame_"+diag.ID).contentWindow;
	...
	pw.$it618("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	pw.$it618("_AlertBGDiv").style.dispaly="";
	var win = pw.$it618("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
...
PageOnLoad=function (){
	...
	if(d){
		try{
			d.ParentWindow.$it618D = d;
			d.ParentWindow.$it618DW = d.Window;
			var flag = false;
			if(!this.AlertFlag){
				...
				if(!flag){
					$it618E.getTopLevelWindow().$it618("_DialogBGDiv").style.opacity="0.4";
					$it618E.getTopLevelWindow().$it618("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				$it618E.show($it618E.getTopLevelWindow().$it618("_AlertBGDiv"));
			}
			if(d.ShowButtonRow&&$it618E.visible(d.CancelButton)){
...
Dialog.setPosition=function(){
	...
	{
		pw.$it618("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	//pw.$it618(this.bgdivID).style.width= sw + "px";
	pw.$it618(this.bgdivID).style.height= sh + "px";
}
